...
void View3DGL::initializeGL()
{
    ...
    // Prepara a imagem da textura do chave
    if( !img.load(":/mainwindow/texture/Ouro.png") )
        {
        img = QImage( 16, 16, QImage::Format_RGB888 );
        img.fill( QColor(VIEW2D_COLOR_3UB_OURO).rgb() );
        }
    textures[VIEW3D_IX_TEXTURE_OURO] = QGLWidget::convertToGLFormat( img );

    // Prepara a imagem da textura do buraco
    ...
    // Prepara a imagem da textura do tijolo
    if( !img.load(":/mainwindow/texture/Chao.png") )
        {
        img = QImage( 16, 16, QImage::Format_RGB888 );
        img.fill( QColor(VIEW2D_COLOR_3UB_CHAO).rgb() );
        }
    textures[VIEW3D_IX_TEXTURE_CHAO] = QGLWidget::convertToGLFormat( img );

    // Prepara a imagem da textura do chao
    if( !img.load(":/mainwindow/texture/Tijolo.png") )
        {
        img = QImage( 16, 16, QImage::Format_RGB888 );
        img.fill( QColor(VIEW2D_COLOR_3UB_TIJOLO).rgb() );
        }
    textures[VIEW3D_IX_TEXTURE_TIJOLO] = QGLWidget::convertToGLFormat( img );
